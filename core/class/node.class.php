...
class node
{
	...
	public $gettingResources = 0, $gettingComponents = 0, $gettingUnits = 0, $gettingModules = 0, $gettingTechnologies = 0;
	public $checkingResources = 0, $checkingCraft = 0, $checkingTrain = 0, $checkingBuild = 0, $checkingResearch = 0, $checkingCombat = 0;
	...
	{
//		if($this->gettingResources){return;}
//		else{$this->gettingResources = 1;}
	...
	{
//		if($this->gettingTechnologies){return;}
//		else{$this->gettingTechnologies = 1;}
	...
	{
//		if($this->gettingModules){return;}
//		else{$this->gettingModules = 1;}
	...
	{
//		if($this->gettingComponents){return;}
//		else{$this->gettingComponents = 1;}
	...
	{
//		if($this->gettingUnits){return;}
//		else{$this->gettingUnits = 1;}
	...
	public function removeModule($slotId)
	{
		...
		if (isset($module['module']))
			if ($module['module']>-1)
			{
				...
				if (!$row['count'])
				{
					$moduleSetting = $game['modules'][$this->data['faction']][$module['module']];
					if (($moduleSetting['type'] == 'extend' && $this->resources[$moduleSetting['extendResource']]['value'] >= $moduleSetting['ratio']) || $moduleSetting['type'] != 'extend')
					{
						if($moduleSetting['type'] == 'extend')//开发度即时减少
						{
							$this->resources[$moduleSetting['extendResource']]['value'] -= $moduleSetting['ratio'];
							$db->query('update resources set value="'.$this->resources[$moduleSetting['extendResource']]['value'].'" where node="'.$this->data['id'].'" and id="'.$moduleSetting['extendResource'].'"');
							if ($db->affected_rows()==-1) $ok=0;
						}
						$this->getQueue('build');
						$lastBuild=count($this->queue['build'])-1;
	...
	public function cancelModule($slotId)
	{
		...
		if (isset($entry['start']))
		{
			...
			{
				$moduleSetting = $game['modules'][$this->data['faction']][$entry['module']];
				if($moduleSetting['type'] == 'extend')//开发度即时增加
				{
				...
				if ($component['requirementsData']['ok'])
					if ($this->resources[$game['components'][$this->data['faction']][$componentId]['storageResource']]['value']>=$game['components'][$this->data['faction']][$componentId]['storage']*$quantity)
					{
						...
						if ($component['costData']['ok'])
						{
							...
							$id = $db->get_primary_id('craft');//2014-07-25  原获得主键的写法是错的
...
//    $id=$row['id']+1;
	...
	{
//		if($this->checkingResources){return;}
//		else{$this->checkingResources = 1;}
	...
	{
//		if($this->checkingResearch){return;}
//		else{$this->checkingResearch = 1;}
	...
	{
//		if($this->checkingBuild){return;}
//		else{$this->checkingBuild = 1;}
		...
		foreach ($this->queue['build'] as $entry)
		{
			...
			if ($entry['end']<=$time)
			{
				if ($this->modules[$entry['slot']]['module']==-1)//build module
				{
					...
					if ($game['modules'][$this->data['faction']][$entry['module']]['type'] == 'extend')//静态资源需要即时增加
					{
	...
	{
//		if($this->checkingCraft){return;}
//		else{$this->checkingCraft = 1;}
	...
	{
//		if($this->checkingTrain){return;}
//		else{$this->checkingTrain = 1;}
